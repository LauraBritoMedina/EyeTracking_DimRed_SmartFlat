...
def exec_(path, pathR, clf):
    ...
    for a in range(0,6):
        ...
        for N in range(1, 44):
            ...
            auxY = []
            auxZ = []
            for j in range(1,31): #Trials with the same conditions (what varies is the content of training and testing sets)
                X, Y, Z = pca.exec_(a, tp[a],c[a],N,path,clf)
                auxX.append(X)
                auxY.append(Y)   
                auxZ.append(Z)
    ...
    print('PCA Results Correctly Saved!!!! ')
